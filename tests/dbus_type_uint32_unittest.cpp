...
TEST_CASE("Unmarshall uint32 little endian from MessageIStream")
{
    ...
    TestUnmarshallFromMessageIStream<uint32_t, Type::Uint32>(__LITTLE_ENDIAN, value, dbusType);
}
...
TEST_CASE("Unmarshall uint32 big endian from MessageIStream")
{
    Type::Uint32 dbusType;
    const uint32_t value = 257978445;
    TestUnmarshallFromMessageIStream<uint32_t, Type::Uint32>(__BIG_ENDIAN, value, dbusType);
}
...
TEST_CASE("Marshall and unmarshall uint32")
{
    ...
    TestUnmarshallFromMessageIStream<uint32_t, Type::Uint32>(value, dbusType, istream);
}
