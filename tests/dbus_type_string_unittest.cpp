...
{
    TestUnmarshallFromMessageIStream("Control characters \r\n\n\r\b \xc2\xa3 \xf0\x9f\x92\xa9");
}
...
TEST_CASE("Truncated stream")
{
    for (const auto endian : { __LITTLE_ENDIAN, __BIG_ENDIAN })
    {
        ...
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 19),
                                "OctetBuffer index out of range error");
        }
        SECTION("Remove some of string")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 16),
                                "Read string error: Not enough data in stream");
        }
        SECTION("Remove string body")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 4),
                                "Read string error: Not enough data in stream");
        }
        SECTION("Remove some of length 3")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 3),
                                "OctetBuffer copy error: Not enough data in buffer");
        }
        SECTION("Remove some of length 2")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 2),
                                "OctetBuffer copy error: Not enough data in buffer");
        }
        SECTION("Remove some of length 1")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 1),
                                "OctetBuffer copy error: Not enough data in buffer");
        }
        SECTION("Remove everything")
        {
            REQUIRE_THROWS_WITH(TestUnmarshallFromMessageIStream(__LITTLE_ENDIAN, "Not UTF-8 \xc2. 0\xf0", 0),
                                "OctetBuffer copy error: Not enough data in buffer");
        }
    }
}
