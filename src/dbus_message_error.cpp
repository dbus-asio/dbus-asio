...
DBus::Message::Error::Error(uint32_t serial, const std::string& name,
...
DBus::Message::Error::Error(const DBus::Type::Struct& header,
    ...
    : Message::Base(header, body)
{
    ...
    Log::write(Log::WARNING,
        "DBus :: Received an error from message serial #%d : %s\n",
        m_SerialReplyingTo, m_Message.c_str());
}

uint32_t DBus::Message::Error::getSerialOfReply() const
{
    return m_SerialReplyingTo;
}

std::string DBus::Message::Error::getMessage() const { return m_Message; }

std::string
DBus::Message::Error::marshall(const std::string& destination) const
{
