...
void DBus::Introspectable::Introspection::addInterface(const Interface& face) { m_Interfaces.push_back(face); }
...
void DBus::Introspectable::Interface::addMethod(const Method& method) { m_Methods.push_back(method); }

void DBus::Introspectable::Interface::addProperty(const Property& property) { m_Properties.push_back(property); }

void DBus::Introspectable::Interface::addSignal(const Signal& signal) { m_Signals.push_back(signal); }
...
// Property
//
DBus::Introspectable::Property::Property(const std::string& name, const std::string& type, size_t access)
    : m_Name(name)
    , m_Type(type)
{
    ...
    default:
        DBus::Log::write(Log::WARNING, "Invalid access passed to property. Assuming read only.");
        m_Access = "read";
        break;
    }
}
...
// Signal
//
DBus::Introspectable::Signal::Signal(const std::string& name, const std::string& type)
    : m_Name(name)
    , m_Type(type)
{
}
...
// Method
//
DBus::Introspectable::Method::Method(const std::string& name, const std::string& in_params, const std::string& out_params)
    : m_Name(name)
