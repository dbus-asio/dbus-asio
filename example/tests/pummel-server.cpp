...
void s_signal_handler(int signal_value) { gInterrupted = true; }
...
void testServer()
{
    ...
    // empty string. Some apps (like d-feet) will issue a GetAll before the
    // standard method call you've requested, and then wait for a reply, making
    // them appear slow.
    native.registerMethodCallHandler(
    ...
    native.callRequestName(
        ...
        [](const DBus::Message::MethodReturn& msg) {
            ...
            std::cout << "Now running as biz.brightsign (" << result << ")"
                      << std::endl;
        ...
        [](const DBus::Message::Error& msg) {
            std::cout << "ERROR FROM callRequestName : " << msg.getMessage()
                      << std::endl;
        });
    ...
    DBus::Message::MethodCall concat(
        DBus::Message::MethodCallIdentifier("/", "biz.brightsign.test", "concat"),
        params);
    native.sendMethodCall(
        ...
        [&bRunLoop](const DBus::Message::Error& msg) {
            std::cout << "Terminating because concat has returned an error."
                      << std::endl;
            bRunLoop = false;
        });
