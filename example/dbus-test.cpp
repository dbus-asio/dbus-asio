...
//	DBus::Log::setLevel(DBus::Log::WARNING);
// TODO: This should get removed, but _something_ i not buffering the method calls below
...
//sleep(3);
    ...
    native.registerMethodCallHandler("org.freedesktop.DBus.Properties.Get", [&](const DBus::Message::MethodCall& method) {
        DBus::Message::MethodReturn result(method.getSerial());
        // TODO: return Serial should be in ctor for error and return - and FIRST PARAMETER
        ...
        //result.m_Parameters.add(DBus::Type::String("The result of " + interface + " of " + propertyName + " is 42. Always 42!"));
        ...
        if (propertyName == "p1") {
            ...
            DBus::Message::Error err(method.getSerial(), "biz.brightsign.Error.InvalidParameters", "Parameter is not p1");
...
// TODO: finish org.freedesktop.DBus.Properties.GetAll to return the correct data as name:value pair
// looks wrong, but it doesn't crash. Whereas the a(sv) is right, but causes the sender to terminate
// the connection
#if 1
    // It is recommend you implement this method - even if you only return an empty string.
    // Some apps (like d-feet) will wait for a reply, making them appear slow.
    native.registerMethodCallHandler("org.freedesktop.DBus.Properties.GetAll", [&](const DBus::Message::MethodCall& method) {
        ...
        propertyList.add(s1);
        //native.sendMethodReturn(method.getSerial(), header.destination, header.sender, propertyList);

        //std::string result("p1:123,p2:44");
        std::string result("");
        DBus::Type::Generic body = DBus::Type::String(result);

        // REM: The destination for the incoming message is us, the sender, in this context.
        ...
        //		native.sendMethodReturn(method.getSerial(), header.destination, header.sender, body);
    });
#endif

#if 1
    native.registerMethodCallHandler("biz.brightsign.TestInterface.Echo2", [&](const DBus::Message::MethodCall& method) {
    ...
    native.registerMethodCallHandler("biz.brightsign.TestInterface.Ping", [&](const DBus::Message::MethodCall& method) {
        DBus::Type::Array propertyList;

        propertyList.add(DBus::Type::DictEntry(DBus::Type::String("p1"), DBus::Type::String("two")));
    ...
    // Run a "server"
    native.callRequestName("test.steev", 0, [](const DBus::Message::MethodReturn& msg) {
    ...
    DBus::Message::MethodCall concat(DBus::Message::MethodCallIdentifier("/", "biz.brightsign.TestInterface", "Echo2"), params);
    native.sendMethodCall("test.steev", concat,
        [](const DBus::Message::MethodReturn& msg) {
